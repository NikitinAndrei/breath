...
def envelope(y, rate, threshold):
    ...
    y_mean = y.rolling(window=int(rate/trsh),
                       min_periods=1,
                       center=True).max()
    for mean in y_mean:
        if mean > threshold:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src_root', type=str, default='wavfiles_old',
                        help='directory of audio files in total duration')
    parser.add_argument('--dst_root', type=str, default='Clean_2',
                        help='directory to put audio files split by delta_time')
    parser.add_argument('--delta_time', '-dt', type=float, default=2,
